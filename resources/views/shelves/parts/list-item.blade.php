<a href="{{ $shelf->getUrl() }}" class="shelf entity-list-item" data-entity-type="bookshelf" data-entity-id="{{$shelf->id}}">
    <div class="entity-list-item-image {{ $shelf->coverInfo()->exists() ? 'bg-white has-image' : 'bg-bookshelf' }}" @if($shelf->coverInfo()->exists()) style="background-image: url('{{ $shelf->coverInfo()->getUrl() }}')" @endif>
        @icon('bookshelf')
    </div>
